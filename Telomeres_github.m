...
TstatsTotal = masterFn(inputfolder, date, TstatsTotal); 

save('TstatsTotal.mat', 'TstatsTotal'); 

function TstatsTotal = masterFn(inputfolder, date, TstatsTotal)
    [jitteredImgName, LEDimgName] = getfilenames(inputfolder, date);
...
function [jitteredImgName, LEDimgName] = getfilenames(inputfolder, date)
    for j = 1 : size(inputfolder)

        if contains(inputfolder(j).name, 'j') && contains(inputfolder(j).name, 'G')...
%             && contains(inputfolder(j).name, 'usedthis')
           jitteredImgName = ['/Volumes/Data/Charlie/analysis/' date '/' inputfolder(j).name];
        elseif    contains(inputfolder(j).name, 'cy5') && contains(inputfolder(j).name, 'G')
%             && contains(inputfolder(j).name, '_3') 
           LEDimgName = ['/Volumes/Data/Charlie/analysis/' date '/' inputfolder(j).name];
        end
